...
in {
  options.programs.raycast.enable = lib.mkEnableOption "Raycast launcher";
  ...
  config = lib.mkIf (cfg.enable && isDarwin) {
    home = {
      ...
      activation.configureRaycast = lib.hm.dag.entryAfter ["writeBoundary"] ''
      ...
      file.".config/raycast/setup-instructions.md" = {
        text = ''
